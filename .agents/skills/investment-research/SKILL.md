...
description: 巴菲特-芒格-段永平-李录四大师综合研究框架：当用户给出公司名/代码要求深度研究（如「深度研究 腾讯」）时触发，按数据收集与交叉验证、生意本质、护城河、逆向风险、管理层、行业趋势、估值与安全边际七模块执行，输出含信息丰富度评级与数据抽检的决策备忘录。
user-invocable: true
...
对用户本次输入中解析出的标的（逗号、顿号或空格分隔）进行系统化投资研究分析。
...
使用 subagent 工具（run_in_background: true）启动后台子代理，从网络收集以下数据：
