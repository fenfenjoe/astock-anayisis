---
name: wechat-article
description: 微信公众号文章生产流水线：研究子代理收集素材→作者子代理写深度初稿→编辑子代理+读者子代理并行审阅→定稿，产出可直接发布的公众号文章（含配图与LaTeX公式）。用户给出主题（如"大模型OPD技术解读"）要求写成公众号文章时触发。
user-invocable: true
origin: migrated-from-claude-command
---

# 微信公众号文章：作者-编辑-读者三子代理协作

> 执行环境：DSH（Windows，PowerShell）。文中 bash 代码块若与 PowerShell 语法冲突，执行时转换为等价的 PowerShell 命令；python 命令直接执行。

对从用户本次输入中解析的主题（逗号、顿号或空格分隔）进行深度研究，产出一篇可直接发布的微信公众号文章。三个子代理各司其职：作者写深度初稿，编辑精修结构和表达，读者从目标受众视角审读。
...
单人写作容易"自嗨"——写的人觉得清楚，读的人看不懂。三子代理协作的本质是**强制引入外部视角**。
...
使用 subagent 工具（run_in_background: true）**并行**启动2-3个研究子代理，收集足够的素材：
...
使用 subagent 工具（run_in_background: true）启动**作者子代理**，给出详细的写作指令。

### 作者子代理的Prompt模板

```
你是一位深度技术写作者（Author 子代理），需要写一篇微信公众号文章。
...
初稿完成后，使用 subagent 工具（run_in_background: true）在**同一条消息**中启动编辑子代理和读者子代理。

### 编辑子代理的Prompt模板

```
你是一位资深公众号编辑（Editor 子代理）。请对以下文章进行精修审阅。
...
```
你是一位{目标读者画像}（Reader 子代理）。请从读者视角审读以下文章。
