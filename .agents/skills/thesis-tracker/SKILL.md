---
name: thesis-tracker
description: 买入后投资论文追踪的纪律系统：首次为标的建立投资论文（5句话核心论文+假设清单+红线清单+估值锚点），此后按季度/财报追踪检查论文健康度并更新。用户说"给XX建立论文""检查XX的投资论文"或对已持仓公司要求跟踪时触发。
user-invocable: true
...
对从用户本次输入中解析的标的/参数（公司名，逗号、顿号或空格分隔）执行投资论文追踪检查。
...
使用 web_search 获取当前股价、估值指标（PE/PB/股息率）、最新财报核心数据，用于填写估值锚点。如果已有该公司的 `/investment-research` 或 `/investment-team` 报告，优先从中读取。
