...
description: '管理层纵深研究：对指定公司或管理层执行买股票就是买人的深度评估（诚信度、战略执行、资本配置、治理结构、侧面验证）。当标准投研中管理层评分不确定（★★★或以下）、或管理层是核心投资逻辑时，输入公司名或人名加公司名触发。'
user-invocable: true
...
对从用户本次输入中解析出的公司或管理层标的（参数以逗号、顿号或空格分隔）进行管理层深度研究。
...
确认关键人物后，使用 subagent 工具（run_in_background: true）启动多个后台子代理 **并行**收集以下数据：
1. 子代理 1：CEO公开发言与预测记录（股东信、电话会、采访、社交媒体）
2. 子代理 2：资本配置决策记录（并购、回购、分红、新业务投资）
3. 子代理 3：治理结构与薪酬（股权结构、关联交易、高管薪酬）
4. 子代理 4：侧面验证信息（员工评价、客户反馈、行业口碑）
