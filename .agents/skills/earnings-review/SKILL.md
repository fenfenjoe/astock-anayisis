...
description: 财报精读（一手资料深度解读）：对用户指定公司/标的进行财报原文精读——读原文、看变化、听语气、查附注、给结论。触发：用户说"财报精读"/"精读 腾讯 2025Q4"，输入格式"公司名 季度"，如"腾讯 2025Q4"、"PDD 2025年报"。
user-invocable: true
...
对从用户本次输入中解析出的标的/参数（逗号、顿号或空格分隔）进行财报精读分析。
...
使用 subagent 工具（run_in_background: true）启动多个后台子代理 **并行**获取以下原始材料：
