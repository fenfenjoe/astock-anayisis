...
description: 供应链瓶颈猎手：对超级趋势做物理供应链拆解→瓶颈识别（6条标准）→公司筛选（估值硬门槛）→交叉验证→输出瓶颈机会看板，专挖第二三层alpha。用户说"找XX趋势的瓶颈/卡脖子环节""什么会先不够用"时触发。
user-invocable: true
...
对从用户本次输入中解析的超级趋势（逗号、顿号或空格分隔）执行供应链瓶颈扫描与套利机会挖掘。
...
对每个确认的超级趋势执行类似拆解。使用web_search搜索：
...
搜索方式：
- web_search "{瓶颈环节} supplier listed company"
- web_search "{瓶颈环节} manufacturer stock"
- web_search "{瓶颈产品} market share company"
