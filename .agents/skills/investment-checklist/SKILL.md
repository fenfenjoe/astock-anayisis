...
description: 巴菲特买入前六关 Checklist：当用户给出公司名/代码（逗号、顿号或空格分隔，如「茅台 腾讯」）要求评估是否值得买入、跑一遍买入前检查时触发，执行能力圈、好生意、护城河、管理层、安全边际、仓位纪律六关分析，输出六关评分表、镜子测试与通过/否决结论。
user-invocable: true
...
对用户本次输入中解析出的标的（逗号、顿号或空格分隔）执行巴菲特价值投资买入前 Checklist 分析。
...
从用户本次输入中解析出所有公司名称/代码（逗号、顿号或空格分隔）。对每家公司确定：
- 公司全称、股票代码、上市交易所
...
使用 subagent 工具（run_in_background: true）为**每家公司**启动独立的后台子代理进行数据收集（所有公司同时并行启动），每个子代理负责收集：
