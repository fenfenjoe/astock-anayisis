...
description: 深度公司系列（8篇长文拆一家公司）：为指定公司撰写8篇深度长文系列（约12万字），从认知重置到决策框架完整闭环，核心是严苛事实核查与修订流程，发布公众号/视频号。触发：用户说"深度公司系列"/"写《看懂某公司》系列"/"为某公司做教科书级深度研究"。
user-invocable: true
...
为从用户本次输入中解析出的标的/参数（逗号、顿号或空格分隔）撰写一个 8 篇深度长文系列，发布在公众号/视频号等公开渠道。**核心 IP 不是"会写"，而是"会改"——99% 的财经文章在违反本 skill 的事实核查标准**。
...
如果用户说"X 数据不对"，先用 pwsh 命令 / read 工具找原始数据交叉验证：
