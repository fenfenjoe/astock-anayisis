...
| 管理 | 13 个独立 CronCreate | 1 个 task_schedule.json (14 任务) |
| 过期 | 7 天自动过期需重建 | 无过期概念 |
...
- **查看 Dev Loop**：`python .claude/scripts/devloop_scanner.py --status`
- **检测 Dev Loop 变化**：`python .claude/scripts/devloop_scanner.py --delta`

## 调度文件位置

- 任务定义：`.claude/scripts/task_schedule.json`（14 条任务）
- 调度器：`.claude/scripts/task_scheduler.py`
- Dev Loop 扫描器：`.claude/scripts/devloop_scanner.py`
- 运行时状态：`.claude/scripts/scheduler_state.json`（自动创建）
- Dev Loop 快照：`.claude/scripts/devloop_snapshot.json`（自动创建）
