...
description: '对指定行业执行系统化产业链投资研究：验证投资逻辑链、绘制产业链全景、扫描全球上市公司、对头部公司做四大师分析并输出行业级组合配置建议。当用户给出一个投资主题或行业（如AI算力、创新药）要求行业全景扫描或行业级投研时触发。'
user-invocable: true
...
对从用户本次输入中解析出的行业（参数以逗号、顿号或空格分隔）进行系统化产业链投资研究。
...
使用 subagent 工具（run_in_background: true）启动后台子代理，全面搜索该行业所有上市公司。
