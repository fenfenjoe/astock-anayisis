...
description: '行业漏斗筛选：对指定行业或方向执行漏斗式价值投资筛选，从全市场扫描逐层精选到 3 家终选标的，输出投资建议与仓位配置。当用户给出一个行业或投资方向（如AI算力、创新药、机器人）要求筛出值得深度研究的头部公司时触发。'
user-invocable: true
...
对从用户本次输入中解析出的行业/方向（参数以逗号、顿号或空格分隔）执行漏斗式价值投资筛选，从全市场扫描逐层精选到 3 家终选标的。
