...
description: 投资组合审视与优化：解析持仓清单→获取最新数据→单仓位体检→集中度/相关性/机会成本/压力测试→输出调仓建议并更新组合文件。用户提供持仓清单（比例或股数），或说"审视我的组合""调仓建议"时触发。
user-invocable: true
...
对从用户本次输入中解析的标的/参数（持仓清单，逗号、顿号或空格分隔）执行投资组合审视与优化。
...
使用 subagent 工具（run_in_background: true）启动后台子代理，通过 web_search 为每个持仓并行获取：
