...
description: 投研团队（四角色并行分析框架）：对用户指定公司进行团队化投资研究，四角色（商业模式/财务估值/行业竞争/风险评估）并行分析，输出含四维评分与投资建议的完整报告。触发：用户说"投研团队"/"团队研究 某公司"/"全面研究 腾讯"。
user-invocable: true
...
对从用户本次输入中解析出的标的/参数（逗号、顿号或空格分隔）进行团队化投资研究分析。使用 subagent 工具（run_in_background: true）创建真正的多子代理并行研究团队。
...
| B级（信息适中） | 上市不久、覆盖有限 | 每个子代理的推算数据必须标注置信度，team-lead汇总时标注"数据充分度" |
...
使用 subagent 工具（run_in_background: true）并行启动子代理即构成研究团队（DSH 无 TeamCreate，无需显式创建团队）。团队信息：
- 团队名：`{公司名}-research`（英文小写，如 `meituan-research`）
- 牵头角色：`team-lead`（你自己）

### 第三步：定义4个任务

为4个子代理分别定义任务（每个都要有 subject、description）：
...
使用 subagent 工具（run_in_background: true）同时启动4个后台子代理（**必须在同一条消息中并行调用**）：
...
**研究方法**：
- 使用 web_search 工具搜索最新公开信息（财报、行业报告、新闻）
...
**完成后**：
1. 完成分析后将完整分析报告作为最终输出返回（等价于任务 completed）
2. 报告交由 team-lead（你自己）汇总——DSH 子代理结果以最终消息自动返回，无需额外 SendMessage
```
...
全部报告收到后，4个子代理已各自完成并返回结果——DSH 中任务结束即自动结束，无需发送 shutdown_request。
...
无需显式清理——DSH 的子代理在任务结束后自动回收（无 TeamDelete 概念）。

## 重要注意事项

1. **4个子代理必须并行启动**——在同一条消息中调用4次 subagent 工具
2. **子代理通过最终返回消息汇报**——不是文件协作，是消息通信（DSH 子代理以最终输出作为结果交回）
3. **数据准确性**——要求子代理使用 web_search 工具搜索最新数据，关键数据交叉验证
4. **结论要明确**——不回避给出买入/观望/回避建议和具体价格区间
5. **所有分析必须有数据支撑**——附数据来源
6. **耐心等待**——4个子代理研究需要几分钟，实时向用户更新进度
7. **反偏见意识**——team-lead在汇总时必须评估：各子代理的分析是否受限于资料充裕度？是否与市场共识过度趋同？最终报告需包含"信息丰富度评级"和"AI研究局限性声明"
