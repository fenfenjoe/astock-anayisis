---
name: financial-data
description: 财务数据获取与交叉验证规范：当用户需要核实财务数据、要求多源交叉验证、或询问美股/港股/A股财务数据来源与取数方式时触发，规定双来源取数、误差>1%标记、数据呈现格式与常见差异原因。
user-invocable: true
origin: migrated-from-claude-command
---

# 财务数据获取与交叉验证规范

> 执行环境：DSH（Windows，PowerShell）。文中 bash 代码块若与 PowerShell 语法冲突，执行时转换为等价的 PowerShell 命令；python 命令直接执行。
