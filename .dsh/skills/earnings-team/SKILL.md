---
name: earnings-team
description: 财报精读团队（四大师并行解读+公众号发布）：对用户指定公司进行团队化财报精读，段永平/巴菲特/芒格/李录四视角并行解读，编辑润色+读者评审，产出可直接发布的公众号文章。触发：用户说"财报精读团队"/"团队解读 腾讯 2025Q4"，重要公司关键财报需深度+发布时。
user-invocable: true
...
对从用户本次输入中解析出的标的/参数（逗号、顿号或空格分隔）进行团队化财报精读分析。四位大师并行解读财报，编辑润色成文，读者评审把关质量，最终产出可直接发布的公众号文章。
...
使用 subagent 工具（run_in_background: true）启动后台子代理 **并行**获取以下原始材料：
...
使用 subagent 工具（run_in_background: true）在**同一条消息**中启动4个后台子代理。
...
| Skill | 定位 | 何时用 |
|-------|------|--------|
| `/earnings-review` | 单子代理财报精读 | 快速过一遍，只需一个视角 |
| **`/earnings-team`（本Skill）** | **六子代理团队精读 + 公众号发布** | **重要公司的关键财报，需要深度+发布** |
| `/investment-team` | 四子代理全面公司研究 | 首次研究一家公司 |
