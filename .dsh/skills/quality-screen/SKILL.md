---
name: quality-screen
description: 7条指标去劣筛选：当用户要求用硬指标快速筛选或排除公司（个股、行业、指数成分股、主题，逗号/顿号/空格分隔）时触发，按10年平均ROE、5年累计FCF、利息覆盖、毛利率、利润质量、净利率、股本稀释7条指标逐家检验，输出通过/排除/豁免结论与板块总结。
user-invocable: true
...
对用户本次输入中解析出的标的（逗号、顿号或空格分隔）执行去劣指标筛选，快速排除不符合一流公司标准的标的。
...
- 如果输入是行业/市场/主题 → **批量模式**，先执行以下操作：
  1. 用 web_search 搜索该行业/市场/主题下的主要上市公司
...
为每家公司启动独立后台子代理，使用 web_search 搜索以下数据：
