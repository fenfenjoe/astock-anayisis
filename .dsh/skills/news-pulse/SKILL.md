---
name: news-pulse
description: 股价异动快速归因（10分钟情报响应）：4个并行子代理侦察公司事件/监管政策/行业对手/市场情绪，产出事件时间线+异动主因判断+是否触发论文重审。持仓股价单日±5%或一周±10%异动、财报后想搞清市场反应时触发。
user-invocable: true
...
对从用户本次输入中解析的标的/参数（公司名/代码/时间窗口，逗号、顿号或空格分隔）进行最近新闻侦察与异动归因。**这不是深度投研，是情报快速响应**——目标是 10 分钟内回答："这家公司最近发生了什么？股价异动的真因是什么？要不要重审投资论文？"
...
| 等级 | 特征 | 侦察策略 |
|------|------|---------|
| **A 级（信息充裕）** | 大盘股、媒体覆盖广、财报季 | 重点是**降噪和归因**——信息太多反而难找真因，每个子代理要有判断力，过滤掉"重复转述"的二手新闻 |
...
使用 subagent 工具（run_in_background: true）启动后台子代理，无需创建团队：
- 子代理命名前缀：`{公司名}-newspulse`（英文小写，如 `pdd-newspulse`）
- 综合归因由主代理完成
...
**必须在同一条消息中并行调用 4 次 subagent 工具（run_in_background: true）启动 4 个后台子代理**。每个子代理对应一个角色名：company-event-scout / regulatory-watcher / industry-peer-analyst / sentiment-tracker。

每个子代理的 prompt 模板：

```
你是 {公司名} 新闻脉搏侦察组的"{角色中文名}"子代理，负责侦察 {侦察维度} 维度的最近 {N} 天事件。
...
**侦察方法**：
- 优先使用 web_search 搜索时效性查询（关键词加日期或"最近"、"latest"、"2026"）
- 关键事件用 web_search 精读原始来源（公告原文、财报、监管文件）
...
**完成后**：
1. 将完整侦察报告作为子代理的最终结果直接返回（subagent 工具自动收集结果，无需手动标记任务状态）
2. 报告内容即本维度全部侦察产出，供主代理汇总归因
```

### 第六步：实时跟踪进度

- 每收到一份子代理报告，向用户展示该维度的 3 条核心发现
- 等待全部 4 份到齐
- 后台子代理返回报告后自动结束，无需手动关闭
...
DSH 无团队资源需清理：所有后台子代理在返回报告后自动结束。
