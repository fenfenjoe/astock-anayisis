...
description: '未上市公司研究：对指定未上市公司（如蚂蚁集团、小红书、SpaceX、Stripe）执行多子代理并行深度研究，还原公司真实价值。当用户要求研究一家未上市公司的商业模式、财务估值、竞争格局、风险治理、技术与替代数据信号时触发。'
user-invocable: true
origin: migrated-from-claude-command
---

# 未上市公司研究：多子代理并行深度研究框架

> 执行环境：DSH（Windows，PowerShell）。文中 bash 代码块若与 PowerShell 语法冲突，执行时转换为等价的 PowerShell 命令；python 命令直接执行。

对从用户本次输入中解析出的未上市公司标的（参数以逗号、顿号或空格分隔）进行团队化深度研究分析。专为蚂蚁集团、小红书、SpaceX、Stripe 等未上市公司设计。
...
使用 subagent 工具创建研究团队（当前主代理即 team-lead，负责统筹协调、信息拼图、交叉验证与最终报告）：
- 团队标识：`{公司名}-private-research`（英文小写，如 `ant-group-private-research`）
- 主代理角色：`team-lead`

### 第三步：创建6个任务

使用 subagent 工具创建以下6个子代理任务（每个都要有 subject、description、activeForm）：
...
使用 subagent 工具（run_in_background: true）同时启动6个子代理（**必须在同一条消息中并行调用**）：
...
**研究方法**：
1. 使用 web_search 搜索最新公开信息，每个维度至少搜索3-5次，用不同关键词组合
2. 搜索关键词策略：
...
4. 使用 web_search 获取关键文章的全文（不要只看搜索摘要）
...
无需清理团队资源：DSH 子代理为一次性后台运行，全部报告到齐后任务自然结束（如需提前终止仍在运行的子代理，使用 interrupt_agent 中断）。

---

## 重要注意事项

1. **6个子代理必须并行启动**——在同一条消息中调用6次 subagent 工具（run_in_background: true）
