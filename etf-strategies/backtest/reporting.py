...
def render_markdown_report(results, metrics, path, data_start, data_end):
    ...
    lines.append("- **S12 量价情绪多因子**：动量(30%)+量价情绪(25%)+低波动(20%)+量价关系(25%)四因子加权。放量下跌→危险扣分，最高分为负→切货币避险。每日评估。")
    lines.append("- **S13 多因子综合打分**：动量(40%)+低波动(25%)+趋势质量R²(20%)+成交量趋势(15%)四因子加权。选Top-3，最高分为负→切货币。月末调仓。")
    lines.append("- **S14 动态波动率调整动量**：波动率自适应回看期(15-120天)。高波动→短回看快速反应，低波动→长回看过滤噪音。选Top-2等权，每日调仓。")
    lines.append("- **S15 趋势过滤动量增强**：双层趋势过滤(200日MA+20日斜率)+25日动量(R²×年化)打分，过滤后无标的→切货币。选Top-2等权，周度调仓。")
    lines.append("- **S16 金丝雀防御动量**：三层金丝雀预警(债券趋势/波动率/动量宽度)→风险预算(100/70/40/0%)→动量打分选Top-2。月末调仓。")
    lines.append("- **S17 金丝雀防御动量(日频)**：与S16相同逻辑，改为每日评估调仓，反应更快但成本更高。")
    lines.append("- **S18 RSRS增强反转动量**：RSRS Beta z-score过滤+25d/200d反转动量打分。综合得分=25日动量-(200日动量/6)。Top-1，每日调仓。")
    lines.append("- **S19 低相关ETF轮动**：黄金(518880)/纳指(513100)/创业板(159915)/国债(511260)/沪深300(510300)五类低相关资产，25日动量(R²×年化)打分，选Top-1满仓，周度调仓。\n")
