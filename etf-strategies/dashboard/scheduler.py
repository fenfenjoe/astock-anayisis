...
def _trading_calendar_module():
    ...
    with _trading_cal_lock:
        ...
        spec = importlib.util.spec_from_file_location(
            "_trading_calendar_loaded", TRADING_CALENDAR_PATH
        )
...
def task_due_now(task: dict, now: datetime) -> bool:
    ...
    if (
        task.get("days_of_week") is not None
        and now.weekday() not in task["days_of_week"]
    ):
        return False
...
def _exec_claude(task: dict, started: datetime) -> tuple[str, str]:
    ...
    if not claude_bin:
        return (
            "failed",
            "未找到 claude 可执行文件。请安装 Claude Code 或设置 DASHBOARD_SCHEDULER_ENGINE=dsh。",
        )
    try:
...
def _run_dsh_once(cmd: list[str]) -> tuple[str, str]:
    ...
    try:
        ...
        try:
            ...
            return (
                "timeout",
                f"dsh headless 超时（>{CLAUDE_TIMEOUT_SECONDS}s），已终止。",
            )
        except Exception as e:
...
def _exec_dsh(task: dict) -> tuple[str, str]:
    ...
    if found is None:
        return "failed", (
            "未找到 dsh CLI（node + @deepseek-ai/dsh/lib/bin.js）。"
    ...
    try:
        ...
        for attempt in range(1, attempts + 1):
            ...
            print(
                f"[scheduler] dsh 429 限流，{backoff}s 后退避重试 "
...
def _run_prompt(task: dict, run_id: int) -> None:
    ...
    print(
        f"[scheduler] run#{run_id} {task.get('task_id')} → {status} "
        f"({elapsed}s, engine={engine})",
        file=sys.stderr,
    )
...
def run_task(task: dict, trigger: str = "manual", now: datetime | None = None) -> dict:
    ...
    if trigger == "auto":
        ...
        if db.scheduler_window_done(
            task_id, window_key, max_failures=MAX_AUTO_FAILURES_PER_WINDOW
        ):
            return {
                "started": False,
                "run_id": None,
    ...
    run_id = db.scheduler_run_insert(
        task_id, window_key, trigger, now.strftime("%Y-%m-%d %H:%M:%S")
    )
...
def run_task_sync(
    task: dict, trigger: str = "cron", now: datetime | None = None
) -> dict:
    ...
    run_id = db.scheduler_run_insert(
        task["task_id"], wkey, trigger, now.strftime("%Y-%m-%d %H:%M:%S")
    )
    _run_prompt(task, run_id)
    ...
    def _tick(self):
        ...
        if (
            ...
            try:
                ...
                if r.get("imported", 0):
                    print(
                        f"[scheduler] report rescan imported {r['imported']} new/changed"
                    )
            except Exception as e:
        ...
        for t in scoped_tasks():
            ...
            if db.scheduler_window_done(
                t["task_id"], wkey, max_failures=MAX_AUTO_FAILURES_PER_WINDOW
            ):
                continue  # 该时间窗已成功/超时/失败达上限（幂等 + 防重试风暴）
            result = run_task(t, trigger="auto", now=now)
            if result.get("started"):
                print(
                    f"[scheduler] auto dispatch {t['task_id']} → run#{result['run_id']}"
                )
...
def start() -> None:
    ...
    try:
        ...
        if n:
            print(
                f"[scheduler] start: 回收 {n} 条中断遗留的 running 记录 → failed",
                file=sys.stderr,
            )
    except Exception as e:
